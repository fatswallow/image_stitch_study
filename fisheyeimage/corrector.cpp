...
Mat corrector::heavenAndEarthCorrect(Mat imgOrg, Point center, int radius, double startRadian, CorrectType type)
{
	...
	case Forward:
		...
		for (int j = top; j < top + 2 * radius; j++)
		{
			...
			{
				if (pow((double)i - center.x, 2) + pow((double)j - center.y, 2) > pow((double)radius, 2))
					continue;
...
Mat corrector::getCorrectImage(correctParameters params, correctMethod method)
{
	Mat resultImage;
	switch (method)
	{
